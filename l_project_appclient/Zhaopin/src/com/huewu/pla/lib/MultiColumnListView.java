 ...
 * @date 2012-11-06
 */
public class MultiColumnListView extends PLA_ListView implements OnScrollListener{
	...
	protected PullToRefreshFooter footer;
...
//			MyLogger.i(TAG, "position ##col.getIndex()" + position + "&&" + col.getIndex() + ":" + col.mColumnLeft);
		}
	}
...
//		MyLogger.i(TAG, "lastVisiblePos##" + lastVisiblePos);
		if( lastVisiblePos - getHeaderViewsCount() < mColumnNumber )
			return mColumns[lastVisiblePos - getHeaderViewsCount()];
	...
	public void onLoadMoreComplete(int loadMoreState){
		loadingMoreComplete = true;
		footer.setState(loadMoreState);
		state = State.PULL_TO_REFRESH;
	}
	public boolean isLoadingMoreComplete() {
		return loadingMoreComplete;
	}


	@Override
	public void onScrollStateChanged(PLA_AbsListView view, int scrollState) {
		doLoadMore(view, scrollState);
	}
	public void doLoadMore(PLA_AbsListView view, int scrollState) {
		if(scrollState == SCROLL_STATE_IDLE && getCount() > 0 
				&& footer.getState() != PullToRefreshFooter.STATE_LOADFULL 
				&& footer.getState() != PullToRefreshFooter.STATE_NULL){
            //判断滚动到底部   
            if(view.getLastVisiblePosition() == (view.getCount() - 1) && loadingMoreComplete){
        		loadingMoreComplete = false;
        		footer.setState(PullToRefreshFooter.STATE_LOADING);
	...
	public void setOnLoadMoreListener(final OnLoadMoreListener listener){
		...
		if(listener != null){
			...
			footer = new PullToRefreshFooter(getContext());
