...
import com.qianniu.zhaopin.app.adapter.IScrollingTabsAdapter;
...
public class ScrollableTabView extends HorizontalScrollView implements
    ...
    private IScrollingTabsAdapter mAdapter;
    ...
    private final int mDividerColor = 0xFFFFFF;
    ...
    public void setAdapter(IScrollingTabsAdapter adapter) {
    ...
    public void onPageSelected(int position) {
    	MyLogger.i("test", "position==" + position);
        selectTab(position);
    }
    ...
    private void selectTab(int position) {
        ...
        for (int i = 0, pos = 0; i < mContainer.getChildCount(); i += 2, pos++) {
            ...
            if (tabIds != null && tabIds.length >= 0) {
                for (int j = 0; j < tabIds.length; j++) {
                	TextView tv = (TextView) tab.findViewById(tabIds[j]);
                    if (pos == position) {
                    	tv.setTextColor(mContext.getResources().getColorStateList(
    							R.color.resume_button_color));
                    } else {
                    	tv.setTextColor(mContext.getResources().getColorStateList(
    							R.color.list_title));
                    }
        ...
        if (activity instanceof ExposureWageGetCountActivity) {
            ((ExposureWageGetCountActivity) activity).showDoSubmit(position);
        }
